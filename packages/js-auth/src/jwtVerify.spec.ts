...
describe('jwtVerify', () => {
  ...
  it('should cache in-memory the response from jwks.json', async () => {
    ...
    const jsonwebtokenDecoded = jwt.decode(accessTokenWithFakeKid, {
      complete: true
    })

    const verification1 = await jwtVerify(accessTokenWithFakeKid, {
      ignoreExpiration: true
    })

    const verification2 = await jwtVerify(accessTokenWithFakeKid, {
      ignoreExpiration: true
    })
